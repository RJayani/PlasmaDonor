...
def check(email):
    url = "https://8z2u6ditci.execute-api.ap-south-1.amazonaws.com/login?email="+email
...
def register():
    ...
    print(x)
    params = "name="+x[0]+"&email="+x[1]+"&phone="+x[2]+"&city="+x[3]+"&covid="+x[4]+"&bloodgroup="+x[5]+"&password="+x[6]
    
    if('errorType' in check(x[1])):
        url = "https://xmlrwu8a94.execute-api.ap-south-1.amazonaws.com/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://y3h1koqev5.execute-api.ap-south-1.amazonaws.com/getbloodgroupcount"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    print(address)
    url = "https://udsumthszi.execute-api.ap-south-1.amazonaws.com/getpersonwithgroup?bloodgroup="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
